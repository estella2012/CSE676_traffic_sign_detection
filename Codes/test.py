...
import numpy as np
def images(sign):
  filenames=glob.glob('../content/drive/MyDrive/traffic/STest/'+str(sign)+'/*')
...
def detect(save_img=False):
    ...
    #  if i==75:
    #    sign=i+1
    direc = "/content/drive/MyDrive/traffic/STest/"
    ...
    # for path, img, im0s, vid_cap in dataset:
        ...
        if len(pred[0]) > 0:
            dets=pred[0]
            det=dets[0]
            c=int((det[-1].cpu().numpy()))
            #print("det = ", det, ", det[-1] = ", det[-1], ", c = ", c)
        tot+=1
        t+=(t2-t1)
        #print(det)
        '''
