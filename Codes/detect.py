...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            txt_path = str(save_dir / 'detections' / p.stem) + ('_%g' % dataset.frame if dataset.mode == 'video' else '')
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    number = int(n)
                    detection = str(names[int(c)])
                    rows.append([filename, names[int(c)], number])
            ...
            try:
                if dataset.mode == 'images':
                    im0 = cv2.putText(im0, (30,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                else:
